 ...
 * @author sanz
 */
public class ESOrangeDelfin30 extends ESOrange {
	...
	public String getPlanURL() {
		return "http://movil.orange.es/tarifas-y-ahorro/contrato/delfin-30/";
	}
		...
		for (Chargeable chargeable : data) {
			if (chargeable.getChargeableType() == Chargeable.CHARGEABLE_TYPE_CALL) {
				...
				if (call.getContact().getMsisdnType() == MsisdnType.ES_SPECIAL_ZER0) {
					...
					boolean insidePlan = secondsTotal <= maxSecondsMonth && recipients.size() <= maxRecipients; 
					if (!insidePlan) {
